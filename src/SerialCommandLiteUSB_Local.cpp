...
SerialCommand::SerialCommand()
  : commandList(NULL),
	...
	altTerm('\r'),			// alternate terminator for commands, carriage return character
    last(NULL)
{
...
void SerialCommand::readSerial() 
{
	if (timerIsStarted)
	{
		if (millis() - timeoutTimer > SERIAL_INPUT_TIMEOUT_PERIOD)
		{
			timeout = true;
			inChar = term;
		}
	...
	while (Serial.available() > 0 || timeout)
	{
		...
		{     // Only printable characters into the buffer
			...
			} //(bufPos < SERIALCOMMAND_BUFFER) 
			else 
			{
				#ifdef SERIALCOMMAND_DEBUG
				Serial.println("Line buffer is full - increase SERIALCOMMAND_BUFFER");
				#endif
			} //(bufPos < SERIALCOMMAND_BUFFER) else
			timerIsStarted = true;
			timeoutTimer = millis();
		} //else if (isprint(inChar))
		inChar = NULL;
	} //(Serial.available() > 0 || timeout)
}
